        ...
        if(isset($_COOKIE['Campaigners_ID'])){
            if(DB::query('SELECT user_id FROM member_login_tokens WHERE token=:token', array(':token'=>sha1($_COOKIE['Campaigners_ID'])))){
                    ...
                    if(isset($_COOKIE['Campaigners_ID_'])){
                        ...
                        setcookie("Campaigners_ID", $token, time() + 60 * 60 * 1, '/', NULL, NULL, TRUE); //setting cookie to 1 hour
                        setcookie("Campaigners_ID_", '1', time() + 60 * 30, '/', NULL, NULL, TRUE); //setting (rest) cookie to 30 minutes
                        return $userid;
                    }
            }
        }else{
