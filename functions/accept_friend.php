...
if(isset($_POST['id'])){
    ...
    if(DB::query('SELECT 1 FROM members WHERE id=:id',array(':id'=>$id))){ // Check if user exists
        ...
        }else if(!DB::query('SELECT 1 FROM friends WHERE sender_id=:sender_id AND receiver_id=:receiver_id',array(':receiver_id'=>$user_id,':sender_id'=>$id))){ // Check if user received a friend request
            http_response_code(403);
            $response->message = "Friend request does not exists";
            $response->status = "Forbidden";
            $response->code = "403";
            $response->time = date('Y-m-d H:i:s');
